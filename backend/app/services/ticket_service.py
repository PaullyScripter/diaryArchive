...
async def get_ticket(
    ...
    return {"data": _build_ticket_response(ticket, page=page, per_page=per_page)}


async def assign_ticket(ticket_id: str, admin_id: str, admin_username: str) -> dict:
    ticket_repo = TicketRepository()
    ticket = await ticket_repo.get_by_id(ticket_id)
    if ticket is None:
        raise NotFoundException("Ticket not found")
            ...
            if media.get("is_private"):
                ...
                msg_doc["media_url"] = await asyncio.to_thread(
                    lambda: client.presigned_get_object(
                        settings.minio_bucket, stored_path, expires=timedelta(hours=24)
                    )
                )
            else:
                msg_doc["media_url"] = (
                    f"{settings.minio_endpoint}/{settings.minio_bucket}/{stored_path}"
                )
        ...
        await user_repo._collection.update_one(
            {"_id": target_user["_id"]},
            {
                ...
                "$unset": {
                    "banned_at": "",
                    "ban_reason": "",
                    "bio_warning_deadline": "",
                    "bio_warning_reason": "",
                    "bio_warning_awaiting_review": "",
                    "bio_edit_banned_until": "",
                    "username_warning_deadline": "",
                    "username_warning_reason": "",
                },
            },
        )
...
async def get_user_appeals(user_id: str) -> list[dict]:
    ...
    appeal_tickets = [t for t in tickets if t.get("category") == "account_help"]
    return [_build_ticket_summary(t) for t in appeal_tickets]


def _build_ticket_response(ticket: dict | None, page: int = 1, per_page: int = 50) -> dict:
    if ticket is None:
        return {}
    ticket_user_id = str(ticket.get("user_id", "")) if ticket.get("user_id") else ""
    all_messages = ticket.get("messages", []) or []
    total_messages = ticket.get("messages_count", len(all_messages))
    total_pages = max(1, (total_messages + per_page - 1) // per_page)
    page = min(max(page, 1), total_pages)
    start = (page - 1) * per_page
    selected = all_messages[start : start + per_page]
    messages = []
    for m in selected:
    ...
    return {
        ...
        "assigned_admin_id": str(ticket["assigned_admin_id"])
        if ticket.get("assigned_admin_id")
        else None,
